...
namespace Mozart
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            string[] soundfiles = new string[16];

            for (int i = 0; i < 16; i++)
            {
                int dice = r.Next(1, 7) + r.Next(1, 7) - 2;
                soundfiles[i] = @"C:\Users\zbcjaso\Mozart\Wave\Wave\M" + minuet[dice, i] + ".wav";
            }
            for (int i = 0; i < 16; i++)
            {
                int dice = r.Next(1, 7)-1;
                soundfiles[i] = @"C:\Users\zbcjaso\Mozart\Wave\Wave\T" + trio[dice, i] + ".wav";
            }
            SoundPlayer sp = new SoundPlayer();

            foreach (string element in soundfiles)
            {
                sp.SoundLocation = element;
                sp.Load();
                sp.PlaySync();
            }
        }
